  ...
  <div class="container">
    ...
    <?php 
       ...
       if (isset($_POST["checkout"])) {
        ...
        $sql = $koneksi->query("INSERT INTO pembelian (id_pelanggan, id_prov, tanggal_pembelian, total_pembelian, tarif, alamat,status)  VALUES ('$id_pelanggan', '$id_ongkir', '$tanggal_pembelian', '$total_pembelian', '$tarif', '$alamat','Pending')" );

        $last = mysqli_query($koneksi, "SELECT * FROM pembelian ORDER BY id_pembelian DESC LIMIT 1");
        $pembelian  = mysqli_fetch_array($last);

        //MENDAPATKAN ID_PEMBELIAN YANG BARUSAN TERJADI
        $pembelian_barusan = $koneksi->insert_id;

        foreach ($_SESSION['keranjang'] as $id_produk => $jumlah) {
          $sql2 = $koneksi->query("INSERT INTO pembelian_produk (id_pembelian, id_produk, jumlah) 
                           VALUES ('$pembelian[id_pembelian]', '$id_produk', '$jumlah')");
        }
        ...
        if($sql2 === true){
          ...
          echo "<script>location='nota.php?id_pembelian=$pembelian[id_pembelian]';</script> " ;
        } else {
